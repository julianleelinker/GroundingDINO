...
def distribute_files_into_folders(
    ...
    for i in range(1, len(files)//files_per_folder + 2):
        pathlib.Path(dest_dir + f'-split_{i}').mkdir(parents=True, exist_ok=True)
    ...
    for index, filename in tqdm.tqdm(enumerate(files), total=len(files)):
        ...
        dest_path = pathlib.Path(dest_dir + f'-split_{folder_index}') / filename
