...
output_root = "/mnt/lighthouseACD/QAed-data/bbox/dedup-new/Taiwan_Power"
slice_root = "/mnt/lighthouseACD/QAed-data/bbox/dedup/Taiwan_Power"
# slice_folder_list = pathlib.Path(slice_root).glob("*")
# slice_folder_list = list(pathlib.Path(slice_root).glob("*/*/*"))
slice_folder_list = list(pathlib.Path(slice_root).glob("*/*"))
# slice_folder_list = [x for x in slice_folder_list if x.is_dir()]
# slice_folder_list = slice_folder_list[:1]
print(f"{slice_folder_list=}")
...
for slice_folder in tqdm.tqdm(slice_folder_list):
    ...
    for image in tqdm.tqdm(image_new_annos):
        ...
        dst_image_path = pathlib.Path(org_image_path.replace(slice_root, output_root))
        if not dry_run:
