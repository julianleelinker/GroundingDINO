...
def plot_boxes_to_image(image_pil, tgt, show_id=False, color=(0, 0, 0)):
    H, W = tgt["size"]
    boxes = tgt["boxes"]
    labels = tgt["labels"]
    ...
    random_color = color is None # Use random color if None is provided
    for id, (box, label) in enumerate(zip(boxes, labels)):
        ...
        # draw.text((x0, y0), label_text, fill=color)
        ...
        # bbox = draw.textbbox((x0, y0), label_text)
...
def load_model(model_config_path, model_checkpoint_path, cpu_only=False):
    ...
    # print(load_res) # Removed print statement
    _ = model.eval()
    return model
...
def get_grounding_output(model, image, caption, box_threshold, text_threshold=None, with_logits=True, cpu_only=False, token_spans=None):
    ...
    # print(caption) # Removed print statement
    with torch.no_grad():
    ...
    if token_spans is None:
        ...
        # build pred
        pred_phrases = []
        scores = [] # Added scores list
        for logit, box in zip(logits_filt, boxes_filt):
            pred_phrase = get_phrases_from_posmap(logit > text_threshold, tokenized, tokenlizer)
            if with_logits:
                scores.append(logit.max().item()) # Added score calculation
        ...
        # Note: scores are not calculated in token_spans mode in either file,
        # returning None or empty list might be appropriate if consistency is needed.
        # For now, matching the gpt script which doesn't use scores in this branch.
        scores = [] # Or None, depending on desired behavior
...
if __name__ == "__main__":
    ...
    # run model
    boxes_filt, pred_phrases, _ = get_grounding_output(
