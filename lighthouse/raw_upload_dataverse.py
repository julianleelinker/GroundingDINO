...
def check_and_execute(data_path, command):
    ...
    if not upload.exists():
        print(f"COCO {data_path} not uploaded, Running command:")
        print(command)
        try:
        ...
        print(f"COCO {data_path} already uploaded")
        return False
...
def main(conda_env, prefix):
    ...
    raw_root = "/mnt/lighthouseACD/raw-images/hand0625"
    file_path_list =[
        p for p in pathlib.Path(raw_root).glob('*/*') if p.is_dir()
    ...
    while n_batch < 50:
        ...
        for i, file_path in tqdm.tqdm(enumerate(file_path_list), total=len(file_path_list)):
            ...
            command = f'conda run -n {conda_env} python tools/import_dataset_from_local.py -host https://visionai.linkervision.ai/dataverse/curation -e julianlee@linkervision.com -p {DATAVERSE_PASSWORD} -s 697aa90b-00d0-4455-8863-bd2ad70a93e7 -project 497 --folder {file_path} -name {dataset_name} -type raw_data -anno image'
